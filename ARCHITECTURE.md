...
- `TestDoctorReportsOffAllowlistAPIBase` (doctor names an off-allowlist host before a command refuses mid-run) — `internal/cmd/doctor_test.go:1921`
...
- `architectureLinkWarnings` (doctor advisory section) — `internal/cmd/doctor.go:778`
...
- `loadIfExists` — `internal/cmd/validate.go:364`
...
Read inbound issues *off* the board — the direction [Issue board sync](#issue-board-sync) doesn't cover — so `/dross-inbox` and `/dross-status` can triage work filed by other people. Two faults made that read untrustworthy and both are closed. **Label filters OR rather than AND** on every backend: YouTrack and Jira emit a single disjunctive clause (one `tag:` token, one `labels IN (…)`), while GitHub and the REST backends (forgejo/gitea/gitlab) — whose APIs only ever AND a comma-joined `labels` param — fan out one request per label and union the results by issue number, so `--labels bug,feature` returns issues carrying *either*. A label the tracker has never heard of is **dropped and named on stderr** rather than erroring or silently matching nothing (locked `unknown_label`); the drop is not allowed to degrade into an unfiltered whole-board list, and a failure to read the tracker's label index refuses the query instead of guessing the index is empty. **A board failure is distinguishable from an empty board** (locked `pull_failure_signal`): `--json` emits an envelope `{"issues": […], "error": null|"…"}` and keeps exit 0 — the workflow prompts call `dross issue …` unconditionally on the promise it is a safe no-op, so a non-zero exit would break that contract — human mode says the board is unreachable instead of printing "no new issues on the board", and `--mark` refuses to stamp `last_pull` on a pull that failed, so the next run doesn't skip past the window it never actually read. **The feed also excludes what dross itself filed** (locked `exclusion_basis`): an issue linked in *any* board.json namespace — milestones, phases, quicks, backlog items, task cards — or carrying the `dross` marker label is dropped before the feed is built, so a board dross has mirrored a milestone's worth of artefacts onto returns the handful a human actually filed rather than its own writing. The two bases are deliberately redundant: board.json is branch-local, so a mirror created on a phase branch that never merged is invisible to it, while the marker label travels with the issue. Only the milestone slot is shape-gated — its value can be a numeric forge milestone id, which must not suppress the same-numbered human issue — and an empty id never matches, so an empty task link and an empty forge key cannot meet and blank the whole feed.

- `pullEnvelope` (`{issues,error}`; exit 0 preserved, `last_pull` not stamped on failure) — `internal/cmd/issue.go:1390`
- `FilterKnownLabels` (unknown labels dropped + named; index failure refuses the query) — `internal/forge/forge.go:326`
- `YouTrackClient.buildQuery` / `JiraClient.buildJQL` (single disjunctive clause) — `internal/forge/youtrack.go:766`, `internal/forge/jira.go:249`
- `GitHubClient.ListIssues` (per-label fan-out unioned by number) — `internal/forge/github.go:200`
- `Client.listIssuesByLabel` (same fan-out for forgejo/gitea/gitlab) — `internal/forge/forge.go:679`
- `collectInbound` (mark-free inbound filter, shared with the watch heartbeat; drops linked, dismissed and marker-labelled issues) — `internal/cmd/issue.go:1364`
...
_introduced state-json-branch-safety · extended completion-state-truth · 1ecde33 · extended guard-remedy-ordering · 459ea52_
...
- `CuratedHint` (curated mis-reach → working-invocation table) — `internal/cmd/hints.go:56`
- `Nearest` (Levenshtein fallback over the live command/flag tree) — `internal/cmd/hints.go:70`
...
- `buildUnleashArgs` (mutation runners refuse a leading-dash derived value before exec, returning a nil report rather than an empty one) — `internal/mutation/gremlins.go:416`
...
- `Doctor` (remote + auth_scheme validation) — `internal/cmd/doctor.go:36`
...
- `remoteCombinationWarnings` (runtime-fatal pairings as advisory warnings) — `internal/cmd/doctor.go:709`
- `Remote.AuthUser` (`[remote].auth_user` schema + dotted read/write) — `internal/project/project.go:103`
- `forge.Client.do` (basic scheme + construction-time auth_user check) — `internal/forge/forge.go:832`
...
- `project.BoardFields` (`[board.fields]` nested table, locked `field_config_shape` — same idiom as `board.state_map`) — `internal/project/project.go:211`
...
- `Deferred` (dross deferred list/route/unroute/dismiss) — `internal/cmd/deferred.go:39`
- `collectDeferred` (scan + filter) — `internal/cmd/deferred.go:51`
- `deferredRoute` (stamp target on disk) — `internal/cmd/deferred.go:186`
- `deferredDismiss` (retire to dismissed, someday-only) — `internal/cmd/deferred.go:230`
- `deferredUnroute` (clear target → someday; idempotent, refuses dismissed) — `internal/cmd/deferred.go:283`
...
- `deferredAdd` (`dross deferred add`, `--why` / `--target`, one-command filing) — `internal/cmd/deferred_add.go:64`
- `deferredHome` (current-phase spec when loadable, else the `_project` store; no arm returns "nowhere to put it") — `internal/cmd/deferred_add.go:27`
- `deferredStore` (reserved `_project` store + stable per-item id) — `internal/cmd/deferred_store.go:36`
...
- `danglingTargets` (validate walks the store; reserved `phases/_project` dir flagged; rename repoints store entries) — `internal/cmd/validate.go:353`
...
- `lookupPhaseIssue` (target phase's issue for the relates-to link; absent target warns and relinks later) — `internal/cmd/issue.go:808`
- `forge.IssueLinker` (optional link capability: YouTrack + Jira, absent on GitHub by design) — `internal/forge/forge.go:201`

_introduced deferred-item-routing · 6509930 · extended deferred-triage-gaps · 539d475 · extended deferred-unroute-command · fb24bc2 · extended survivor-lifecycle · a6b366d · extended deferred-add-command · 5e86a7b · extended board-sync-truth · a587471_
...
- `interactionCoverageWarnings` (dross doctor on-demand lint) — `internal/cmd/doctor.go:801`
...
- `forge.BoardClient` (interface) + `forge.NewBoard` (provider dispatch) — `internal/forge/forge.go:166`
- `forge.YouTrackClient` + `NewYouTrack` — `internal/forge/youtrack.go:28`
- `YouTrackClient.EnsureMilestoneEntity` / `SetState` — `internal/forge/youtrack.go:348`
- `forge.JiraClient` + `NewJira` (REST v3, versions, transitions) — `internal/forge/jira.go:28`
- `JiraClient.ListIssues` (current `/search/jql` endpoint; the legacy `/search` was removed) — `internal/forge/jira.go:191`
...
- `openBoard` (resolves client solely from `[board]`) / `syncBacklog` — `internal/cmd/issue.go:140`
- `pushBacklogItems` (id-keyed backlog links, title-checked legacy migration, reusable push seam) — `internal/cmd/issue.go:871`
...
- `issuePhaseSync` (`--status` validated against the set before the enabled short-circuit, then normalised) — `internal/cmd/issue.go:987`
- `TestStateMapsKeyExactlyTheEmittedStatuses` (bidirectional emitted-statuses ↔ state-map-keys gate) — `internal/cmd/board_lifecycle_divergence_test.go:269`
...
- `TestYouTrackErrorSnippetTruncatesAndHints` (transport error paths + backlog/subtask calls driven through httptest fakes, not accepted as network seams) — `internal/forge/youtrack_test.go:411`
- `resolvePhaseIssue` (tracker-first mapping by marker label; board.json is a self-healing cache) — `internal/cmd/issue.go:1053`
- `YouTrackClient.applyTags` (dross marker tags on create *and* update, replace semantics) — `internal/forge/youtrack.go:718`
- `YouTrackClient.CloseIssueAs` (writes the resolved state, then verifies the read-back) — `internal/forge/youtrack.go:278`
- `YouTrackClient.stateField` (field names read from `[board.fields]`, today's literals as defaults) — `internal/forge/youtrack.go:56`
- `syncTasks` (`dross issue task sync`: one issue per plan task, related to its phase's, idempotent through board.json) — `internal/cmd/issue_task.go:112`
- `setBoardState` + `runWarnings` (workflow-field write per provider; each capability gap warned once per RUN, not once per task) — `internal/cmd/issue_task.go:330`
- `YouTrackClient.ensureStateValue` (a missing State value is created in the project-PRIVATE bundle and retried; a shared bundle is never written) — `internal/forge/youtrack.go:554`
- `TestTaskEdgesPairTheBoardStatusWithThePlanStatus` (pins WHICH execute edge emits which task status, against the plan-status write beside it) — `internal/cmd/board_lifecycle_divergence_test.go:338`
...
- `closeBoardIssue` (the one close path per backend: mapped state or plain close, then a read-back on the tracker's verdict) — `internal/cmd/issue.go:1207`
- `syncOneTask` (`issue task sync --status`/`--close`: validated status, verified close, agreement point recorded only after the tracker agrees) — `internal/cmd/issue_task.go:198`
- `statusTaskComplete` (the task lane's own terminal status; readable back off a card, never derived from a plan status) — `internal/cmd/task_lifecycle.go:42`
- `checkMilestoneClosable` (`issue milestone sync --close`; refuses a version bundle / agile board / numeric milestone id by name) — `internal/cmd/issue.go:334`
- `reconcileBacklog` (`issue backlog sync --phase`: closes resolved mirrors, names the unattributable, keeps the link on a refused close) — `internal/cmd/issue.go:668`
- `TestEveryMirrorLaneHasATerminalEmission` (every lane's terminal emission pinned in the prompt corpus, distinct from its working states) — `internal/cmd/board_lifecycle_divergence_test.go:554`
- `classifyReap` (the sweep's verdict: every mirror class decided from the on-disk record, never the card) — `internal/cmd/issue_reap.go:129`
- `discoverReap` (marker-label discovery of dross-authored cards no board.json namespace links) — `internal/cmd/issue_reap_discover.go:92`
- `applyReap` (`issue reap --apply`: per-card failure isolation, prior state journalled before each write) — `internal/cmd/issue_reap_apply.go:112`
- `undoReap` (`issue reap --undo`: the last run replayed backwards through `forge.StateWriter`) — `internal/cmd/issue_reap_undo.go:24`
- `forge.StateWriter` (literal-column writes, bypassing the state map; GitHub deliberately does not implement it) — `internal/forge/forge.go:226`
- `TestEveryBoardNamespaceHasAReapPath` (reflection over board.Board: a new namespace with no reap lane fails by field name) — `internal/cmd/board_lifecycle_divergence_test.go:700`
...
- `reportStrandedMirrors` (doctor's read-only stranded-per-lane advisory; never affects exit status) — `internal/cmd/doctor.go:1678`
...
- `backfillResidue` (doctor's offline advisory naming what backfill cannot close, and why) — `internal/cmd/doctor.go:1620`
...
_introduced milestone-branch-model · extended state-json-branch-safety · extended milestone-stacking · extended milestone-lifecycle-close · ea4df10 · extended guard-remedy-ordering · d59a892_

### Milestone progress reporting

`dross milestone progress` answers, in one deterministic call, the question `/dross-milestone` must resolve before it can act: is there an active milestone, is it finished, and if not what is left. Every arm exits 0 — including "planning" and "nothing done yet" — because a non-zero exit would make a prompt read a normal state as a broken command. Status is reported verbatim from the toml, never normalised or inferred, since the dispatch branches on it first. Doneness is not decided here: it comes from the shared reader in [Phase doneness](#phase-doneness), which measures off each phase's own durable status marker rather than the state history (capped at 50 entries, so it silently forgets older phases) and counts a roadmap slug with no phase directory as not-done, so a milestone cannot close with unbuilt criteria still on it (locked `phases_done_test`). `--json` emits the report as a document for prompts to consume; the plain-text arm narrates the same data for humans and is pinned on both of its list guards in each direction.

- `buildMilestoneProgress` (status + done/total + remaining/unscaffolded; delegates the per-phase call to `phaseIsDone`) — `internal/cmd/milestone_progress.go:90`
- `milestoneProgressReport` (the emitted document; `AllDone` is true only when every listed slug is done *and* there is at least one) — `internal/cmd/milestone_progress.go:23`

_introduced milestone-lifecycle-close · 7f2fd2d · extended completion-record-truth · beb597f_
...
- `Milestone` (CLI) — `internal/cmd/milestone.go:21`
...
- `printScopeSummary` (scoped file list + `(+N more)` overflow) — `internal/cmd/verify.go:729`
...
- `Gremlins.Run` (per-package invocation + path re-prefix) — `internal/mutation/gremlins.go:144`
- `Unmeasured` (typed per-package score exclusion: missing / unreadable / zero-covered) — `internal/mutation/gremlins.go:115`
- `Stryker.Run` — `internal/mutation/stryker.go:66`
- `TestStrykerRunDistinguishesMissingFromUnreadable` (report-read, parse and unknown-status seams killed) — `internal/mutation/stryker_test.go:440`
- `Stryker.runArgs` (npx invocation + workdir knob) — `internal/mutation/stryker.go:163`
- `strykerPin` (exact `@stryker-mutator/core@9.6.1`, shared by the argv and the install hint) — `internal/mutation/stryker.go:150`
...
- `profileCacheVars` (recorded profile first, detection as the fallback that keeps it from being dead config) — `internal/cmd/verify.go:402`
- `TestScratchIsNotInsideTheTree` (the placement the first live run corrected) — `internal/mutation/launcher_scratch_test.go:393`

_introduced c8b346e · extended 01c10f0 · extended context-hygiene · extended self-audit · de8b076 · extended config-trust-hardening · 266a84d · extended mutation-diff-scope · 16d4426 · extended survivor-drain · 87ba203 · extended multilang-verify-signal · a58de69 · extended cache-budget-prune · 1b777e4_
...
- `changes.SetBase` (phase-scoped forked-from record, beside `pr`) — `internal/changes/changes.go:225`
...
- `resolveCompleteBase` (tree → phase ref → `--base`; refuses rather than inferring) — `internal/cmd/phase.go:818`
...
Create, list, number, migrate, complete, and reorder/insert/rename phases on dedicated phase/<id> git branches. **The listing has two readings.** Bare `dross phase list` stays the global cross-milestone directory listing every existing caller depends on; `--milestone <version>` walks that one milestone's roadmap in array order instead, marking a slug with no phase directory `(not scaffolded)` and counting it in the denominator — dropping it would report 1/1 on a half-built milestone. Both readings mark each done phase with a `✓ ` prefix (two spaces when not, so slugs stay column-aligned) and close with an `N/M done` footer, drawn from the shared reader in [Phase doneness](#phase-doneness) rather than a second derivation. A slug carried onto more than one milestone's roadmap — a legitimate re-scope — is **deduped silently**, first occurrence in milestone-version order winning, so it lists once at its earlier position; `dross doctor` names any such slug as an advisory that never changes its exit code, because the ambiguity should be findable without being a fault. Phase identity is the bare slug and order lives solely in the milestone `phases` array (phase.Ordered), so create makes bare-slug dirs and appends to the array, while `phase number` / status / the version patch digit all read the 1-based array position (DisplayNumber) and `phase migrate` converts a legacy NN-slug repo idempotently — skipping the in-flight phase and disambiguating colliding slugs — with phase.Dir resolving old NN-slug ids for permanent back-compat. complete is fast-forward + completion-record write + branch-delete (still no commit to main — the record lands in the machine-local, gitignored `state.json`, so it rides nothing). It is the **sole writer of the completed-state transition**: the cleared `current_phase` plus one history-scan-guarded `completed <id>` entry, re-loaded from disk immediately before the write so recovery's own `merged <id>` survives, written *before* the branch teardown so a failed deletion still leaves the confirmed merge recorded, and cleared only when `current_phase` names the phase being completed. `dross ship` marks the phase `shipped` and leaves `current_phase` set; only a confirmed merge clears it. The run ends by **stating the resulting topology** — the branch HEAD landed on, what was actually deleted per side (`describeTeardown` never claims a deletion that did not happen), and the `<n> commits on <base>, not yet on main` clause from `branchTopology`/`renderTopologyLine`, passed the authoritative recorded base rather than an inferred one; `dross status` prints the same line unconditionally so the answer is standing, not a message that scrolls away. Completion is gated by an **authoritative merge check** (`mergeGate`): it reads the phase's recorded PR number from changes.json — resolving it from origin/<base>'s fetched changes.json (`originRecordedPR`) when the stale post-squash-merge working tree lacks it — and requires the provider (`ship.PRStatusFunc`, authoritative across all 5 ship providers) to report that PR merged — announcing a skip when the provider can't answer or errors — falling back to a `git merge-base --is-ancestor` check that **refuses-when-inconclusive** (a missing/squash-deleted ref or a non-ancestor both refuse) — replacing the old cumulative `completed <id>` breadcrumb, which a later merged phase could drag onto the base and thereby false-complete an unmerged phase; only on a confirmed merge does it delete both the local and the remote phase branch idempotently. `mergeGate` also refuses completion when the provider reports the PR's base was retargeted since it was recorded (`checkBaseRetarget`, comparing `PRStatus.BaseRef` against the recorded base after normalizing `refs/heads/` prefixes) — even when merged is true, since merging into an unexpected base is a fact worth refusing on, not a formality; an empty BaseRef (a provider that can't report one) announces a skipped check rather than raising a false alarm, and the refusal is proven uniform across all 5 ship providers. `create` ADOPTS an existing unstarted phase directory rather than coining a near-identical slug. It used to produce `<slug>-2`, append that to the milestone array and cut a branch for it — so the roadmap entry someone scaffolded by routing a deferred item and the phase they then started were two entries apart by a digit nobody chose. Unstarted is keyed on the three files the loop writes as a phase begins (spec.toml, plan.toml, changes.json), not on the directory being empty: a stray file would make a placeholder look occupied and re-open the coining path. A STARTED phase refuses by name, before anything is created, cut or written, and names the ways through (`phase checkout`, `phase rename`, a different title); adoption keeps the phase's existing slot in the milestone array, since re-appending would renumber every phase between. The lifecycle verbs `insert` / `move` / `rename` edit a phase's array slot and identity through pure splice helpers (InsertRelative / MoveRelative / RenameInArray) and shared plumbing (exactly-one-anchor validation, no-op-before-collision, ship-guard via the origin branch); insert scaffolds with a strict slug (no auto-suffix) and rename moves dir + spec id + array entry + deferred targets + local branch atomically — all leaving every other phase byte-for-byte untouched. `dross phase reconcile` clears the whole backlog in one verb: every phase whose branch still exists with no completion record goes through that same `dross phase complete` — same merge gate, same recorded-base resolution, same guarded switches — and one whose merge cannot be confirmed is reported and skipped while the rest still land. It is a convenience over the gate, never a way around it; a batch that relaxed it would be worse than the chore list it replaces, doing the unsafe thing at N times the rate. Its candidate list is deliberately permissive rather than asking "has its PR merged?" itself, since a second implementation of that check is how the two come to disagree. Completion's branch switch is deferred past *every* refusal — the merge gate, the fetch, and the safety-net push all decide while HEAD is still on `phase/<id>` — so a refused complete moves no local ref and needs no compensating checkout, which would itself be a branch switch that can fail mid-refusal and leave a worse state than not trying.

- `Phase` (CLI) — `internal/cmd/phase.go:24`
- `phaseList` (bare listing: global, cross-milestone, deduped, marked and footered) — `internal/cmd/phase.go:80`
- `listMilestoneRoadmap` (`--milestone <version>`: one roadmap in array order, unscaffolded slugs listed and counted) — `internal/cmd/phase.go:134`
- `phase.Ordered` (array order wins over directory-name sort; a slug on two roadmaps emitted once, at its earlier position) — `internal/phase/phase.go:84`
- `duplicateRoadmapSlugs` (doctor's advisory naming a slug claimed by more than one milestone) — `internal/cmd/doctor.go:674`
- `phaseCreate` — `internal/cmd/phase.go:187`
- `phaseNumber` — `internal/cmd/phase.go:54`
- `phaseMigrate` — `internal/cmd/migrate.go:31`
- `phaseComplete` (branch switch deferred past every refusal path) — `internal/cmd/phase.go:364`
- `phaseReconcile` (one verb over the whole backlog; reports-and-skips a phase whose merge is unconfirmed) — `internal/cmd/phase_reconcile.go:27`
...
- `phase.CreateSlug` (create resolves a title to free / adopt / occupied, never a coined suffix) — `internal/phase/phase.go:213`
...
- `saveTOML` (atomic temp-file + rename write) — `internal/phase/phase.go:476`
...
- `declaredExitValues` (needle guard over `--toolchain`, the per-lane fallback and exit 8 in README / options.md / execute.md — and it asserts the documented exit-code list against `test.go`'s own const block, so a new code cannot ship undocumented) — `internal/cmd/lane_toolchain_docs_test.go:158`
...
A red proof is a recorded commit at which a fixture provably fails, plus the doc that replays it — and a pin naming a commit nobody can fetch is a proof of nothing. `classifyReachability` answers whether any `refs/remotes/origin/*` ref still contains a pinned SHA, and doctor fails on one that rotted; `dross phase red-proof repoint` is the repair, a writer sibling of `red-proof set` rather than a `doctor --fix`, so the diagnostic stays diagnostic. Repointing rewrites the pin to the owning phase's fork point and **the replay doc with it** — a repair that fixed only the record would trade an unreachable-pin issue for a prose-disagrees-with-record one. The doc rewrite keys on the *old pin's value*, replacing every literal occurrence of it (full form or an abbreviated prefix, each preserved at its own width) and nothing else: the live fixture names an unrelated commit in a note, and a rewriter keyed on "any 40-hex run" would eat it. It refuses any target the reachability check does not call reachable, treats an indeterminate verdict (shallow clone, no origin refs) as nothing-to-do rather than as rot, is dry-run by default, and leaves a sound pin byte-identical even under `--apply`. Bare `repoint` scans every discovered pin and repairs only the rotted ones; a phase-id narrows it. A recorded `--replay` command makes the repair *checkable*: it runs in a detached worktree at the proposed commit and the repoint is refused unless the proof still goes red — consent-gated on a fingerprint in the gitignored `local.toml`, because the line arrives from tracked `changes.json` and spawning it is code execution on clone. The three non-red answers are deliberately not one answer: **green** refuses (the proof no longer reproduces there), a replay that could not be **run** refuses (an error is not evidence), while merely **unconsented or unrecorded** repoints and says so — refusing there would make a rotted pin unrepairable on a fresh clone, which is exactly where the single-command promise matters. The pin also survives its own branch's death without anyone typing the verb: ship repoints a pin reachable *only* from the phase ref its squash-merge is about to delete and commits the rewrite onto the phase branch so it rides the phase's own PR, and `phase complete` warns rather than refusing, because a merged phase must never be left uncompletable.

- `classifyReachability` (origin-ref containment; reachable / unreachable / indeterminate) — `internal/cmd/redproof.go:67`
- `discoverRedProofPins` (globs every phase's changes.json) — `internal/cmd/redproof.go:119`
- `redProofRewriteDoc` (byte-faithful old-pin rewrite; count 0 is an error, never a silent success) — `internal/cmd/redproof_doc.go:34`
- `planRedProofRepoint` / `applyRedProofRepoint` (cobra-free decision + doc-then-record write with rollback) — `internal/cmd/redproof_repoint.go:73`
- `classifyReachabilityExcluding` (the doomed-pin predicate: reachable, but only from a ref about to die) — `internal/cmd/redproof_repoint.go:198`
...
- `planRemoteBootstrap` (adapter packages installable, runtimes refused by name, an unreachable host is never a plan) — `internal/cmd/remote_bootstrap.go:96`
- `remoteBootstrap` (the verb: dry-run default, `--apply`, no-op over a provisioned host) — `internal/cmd/remote_bootstrap_cmd.go:27`
- `measuredOnOf` (provenance from the adapters used and the tuning that produced them) — `internal/cmd/verify.go:386`
...
_introduced remote-mutation-runner · extended remote-test-runner · 3055f70 · extended board-task-mirror · 7eaa1f0 · extended remote-toolchain-bootstrap · ab83fce · extended lane-remote-locality · 7cfd0fc_
...
- `finalizeIncompleteRoot` (doctor's distinct verdict) / `incompleteRootHeading` — `internal/cmd/doctor.go:899`
...
_introduced context-hygiene · extended root-robustness · 6d33d3b · extended deferred-add-command · fb07ed1_
...
- `changes.SetPR` (records opened PR number per-phase for the completion merge-gate) — `internal/changes/changes.go:210`
...
- `stack.Profile` / `stack.Load` — `internal/stack/profile.go:29`
- `stack.ContentMatch` (content-sniff gate, `All`=AND / `Any`=OR, case-sensitive) — `internal/stack/profile.go:86`
- `stack.Signals.MatchesFile` (case-insensitive glob matcher) — `internal/stack/profile.go:132`
...
Every surviving mutant a verify run reports carries exactly one state — in-diff, routed, accepted, or unclassified — so pre-existing faults stop being furniture. A survivor's identity is its file, mutation operator, and a hash of the mutated line's *normalized source text*, so it survives line drift anywhere in the file while a genuinely different line at the same position is never mistaken for it; a repeated line is further scoped by its enclosing declaration and occurrence ordinal, so two identical lines in different functions stay separately addressable instead of collapsing into one ambiguous key. When that text is still not unique the match is ambiguous and the acceptance is withheld rather than allowed to suppress the wrong thing. Only an accepted survivor earns silence, recorded with a mandatory reason in the tracked repo-root `.dross/survivors.toml` (entries may share a `category` whose prose is written once); routed debt stays visible, labelled with the destination phase it went to. Acceptances never expire — staleness is structural (subject gone), never time-based — and a stale one is surfaced rather than kept silently forever, and retired **through the CLI** (`dross survivor retire` by key or `--stale`, dropping a category left with no members and writing nothing at all when any key in a multi-key retire fails) so removing an entry never means hand-editing the file. `dross survivor drain` closes the loop for the whole repo: it classifies every survivor in the raw per-package reports, treats an unmeasured package as fatal rather than as zero debt, excludes `testdata/` fixtures, and exits non-zero on any undisposed survivor — and it cannot be satisfied by routing a survivor back to the phase that owns the drain. Kill-vs-accept is decided from derived evidence — per-survivor line coverage plus operator applicability, with the gremlins attribution ceiling dominating killability — and an acceptance reason must cite a concrete checkable justification (a real test, or a shared category backed by a live coverage-vs-gremlins fixture) rather than plausible prose.

- `survivor.Resolve` (text-derived key, ambiguity + subject-gone guards) — `internal/survivor/identity.go:95`
- `scopeOf` (enclosing-declaration + occurrence-ordinal scoping for repeated lines) — `internal/survivor/identity.go:166`
...
- `saveIfValid` (validate-then-write guard) — `internal/cmd/task.go:439`
- `Plan.AddTask` / `Plan.RemoveTask` / `Plan.EditTask` (pure in-memory mutators) — `internal/phase/plan_edit.go:144`
- `Plan.MoveTask` (guarded reposition, anchor-wave adoption + dependent reflow) — `internal/phase/plan_edit.go:230`
- `Plan.NextRunnable` (same-wave tie-break by plan-array position) — `internal/phase/phase.go:381`
...
_introduced task-lifecycle-commands · extended task-reordering · extended cli-surface-sweep · 949b375 · extended completion-record-truth · 7c3f7ff_
...
**A lane may translate its own matched paths into a selector.** `selector = "path" | "dir" | "go-package"` is a closed enum of selector SHAPES, not of runners (locked `selector_styles`): a runner is configured by pointing its lane at whichever shape it accepts, so dross never grows a per-framework mapping engine, and a free-form template was rejected because it would be a second command language to fence and consent for. The matched paths are deduplicated and sorted before translation (locked `selector_derivation`), so the derived line depends on the file set and not on argv order — it lands in the transcript beside the consented command, where it has to diff cleanly. Each lane derives from **its own** paths: `testlane.Selection.Matched` is keyed by lane index, so one lane's files can never scope another lane's run. A lane declaring no selector runs its command byte-identically, which is every lane written before this existed. Consent is unchanged: the grant covers the lane's declared `command` and the selector is appended and shell-quoted, exactly as `testCommandLine` already does for `dross test <selector>` (locked `selector_consent`) — re-fingerprinting per file set would refuse practically every scoped run, and a grant issued before selectors existed keeps working. A matched path that no longer exists is dropped before the spawn (locked `missing_paths`), because `go test ./gone/...` is a hard runner error and a task that deleted a file would read as a failing gate.

**A lane that collected nothing is reported, not failed.** A lane whose selector filtered down to nothing, or which exits with a code it declared in `empty_exit`, is a **selector miss**: printed by name with its derived selector, counted apart from the run's worst outcome, and never folded through `worseOutcome` — one lane finding no tests is not a verdict about code the other lanes did measure. Exit **5** is raised only when *every* lane that reached a spawn decision missed, and it is folded rather than returned so it can never outrank a red or a refused lane. Detection is declarative: with no `empty_exit` only the empty-selector filter can produce a miss, and a runner's **output is never read** (locked `empty_detection`) — matching wording would make dross track every framework's phrasing across every version of it. `dross validate` refuses the codes that already mean something else (`0` success, `255` ssh transport, anything outside 0-255) and refuses an `empty_exit` on a lane with no selector, since a code that can never fire is a user believing they configured something they did not.

**A lane may bootstrap itself before it runs.** `prepare = "<cmd>"` on a `[[runtime.test_lane]]` entry is an optional line that spawns after the tree sync and before that lane's own command, on the same host and through the same transport (locked `prepare_locality`) — a lane that bootstrapped only on the remote would measure different things depending on where it landed. It is per lane and never deduplicated, even when two lanes declare the identical line (locked `prepare_scope`): idempotence is the declared contract, so the repeat is the no-op the user promised, while a dedup cache would make a lane's spawn set depend on which neighbours happened to match. A lane skipped before it spawns — a selector miss or a consent refusal — never prepares (locked `prepare_selector_miss`): no spawn, no bootstrap, and a transcript line for a run that never happened reads as evidence the lane ran. A failed prepare skips **only its own lane's** command and yields exit **7**, ranked above a red suite and below a partial transfer (locked `prepare_failure`) — a bootstrap that failed measured nothing about the code, so reusing exit 1 would make "your prepare is broken" indistinguishable from "your tests are broken", which is the collision the exit taxonomy exists to prevent. The re-tag sits inside the command-exit arms only: an unreachable host still exits 3 and an incomplete rsync still exits 4, because those are facts about the run rather than about the prepare. Consent covers **both** lines under one grant per lane (locked `prepare_consent`): `laneConsentLine` frames prepare and command unambiguously rather than concatenating them — a naive join lets a lane re-split across its two fields keep a grant issued for neither — while a lane declaring no prepare still hashes `Fingerprint(lane.Command)` byte-for-byte, so no grant already written into any machine's `local.toml` stales over a project.toml nobody edited. `dross trust --lane` prints both lines before it writes, and the refusal names the line that actually changed, so a user refused over a rewritten bootstrap is not shown an unchanged command and invited to re-grant a line they never read. `dross test lane edit <name> --prepare` is the one in-place lane edit: it supersedes `lane_edit_surface` for this field alone because remove-then-re-add — the workaround the broader edit surface was deferred in favour of — *drops the lane's grant*, collapsing STALE into ABSENT and losing the only signal that says a line the user approved has since been rewritten. Everything else (match, command, selector, empty_exit) stays remove-then-re-add.
...
- `laneToolchainLine` (`--toolchain` on lane add/edit with clear-back-to-derived, and `lane list` showing every lane's effective toolchain marked derived or overridden) — `internal/cmd/test_lane.go:108`
...
- `testLane` (`dross test lane add|list|remove|edit`, `--selector` / `--empty-exit` / `--prepare`; a removed lane's grant goes with it) — `internal/cmd/test_lane.go:31`
- `laneSelectorRefusal` (lane add validates the selector fields through validate's own rule set *before* the write, so the CLI can never persist a lane `dross validate` would reject) — `internal/cmd/test_lane.go:66`
...
- `laneRunLine` (one line per lane, built from its own live matched paths, printed in the header and spawned on both transports) — `internal/cmd/test.go:680`
- `resolveInstall` (the pure four-arm resolver: recipe argv, declared line, refusal, unknown — a declared line replaces the table entry and is consulted before it) — `internal/cmd/lane_install.go:137`
- `laneInstallRecipes` (the built-in table: package-class rows each naming the runtime they need, plus explicit runtime refusal rows) — `internal/cmd/lane_install.go:106`
- `installArgv` (the ONE place a declared line becomes an argv, as `sh -c <line>`, fenced against a leading dash; a recipe's argv passes through untouched) — `internal/cmd/lane_install.go:187`
- `localInstallFn` (the local install seam; the remote half reuses `remoteExecFn` rather than declaring a second one) — `internal/cmd/lane_install.go:207`
...
- `laneConsentLine` (one grant per lane over a framed prepare+command; byte-identical to the bare command for a lane declaring no prepare, so existing grants survive) — `internal/cmd/trust.go:298`
- `laneConsentRefusal` (names the line that actually changed, refuses that lane alone, and keeps STALE distinct from never-trusted) — `internal/cmd/trust.go:406`
- `testLaneEdit` (`dross test lane edit <name> --prepare`: in-place, reads cobra's `Changed` so omitted and empty stay distinct, and *stales* the grant instead of revoking it) — `internal/cmd/test_lane.go:281`
...
_introduced remote-test-runner · 3055f70 · extended test-lane-config · f01ece7 · extended lane-selector-translation · f7aeb0a · extended lane-prepare-step · c9cdc33 · extended lane-remote-locality · 7cfd0fc_
...
- `Verify` (CLI) — `internal/cmd/verify.go:32`
...
- `configuredAdapters` (`[mutation] adapters` allowlist) — `internal/cmd/verify.go:433`
- `finalizeVerify` (idempotent finalize core — finalized marker + phase-stamped events) — `internal/cmd/verify.go:217`
...
- `printOverallScore` (score printed with its in-scope denominator and uncoverable share) — `internal/cmd/verify.go:695`
...
- `suggestedCommand` (ranked verify→ship→inbox→status) — `internal/cmd/watch.go:134`
