...
func TestNewRunWriteBoundary(t *testing.T) {
	...
	secDir := SecurityDir(root)
	if !strings.HasPrefix(runDir, secDir+string(os.PathSeparator)) {
		t.Fatalf("run dir %q escapes the security dir %q", runDir, secDir)
	}

	// Enumerate every path NewRun created under root and assert each one is the
	// security dir or lives within it — a real path-set check, not a
	// never-triggered negative.
	err = filepath.Walk(root, func(path string, _ os.FileInfo, err error) error {
		if err != nil {
			return err
		}
		if path == root || path == secDir {
			return nil
		}
		if !strings.HasPrefix(path, secDir+string(os.PathSeparator)) {
			t.Fatalf("NewRun touched a path outside .dross/security/: %q", path)
		}
		return nil
	})
	if err != nil {
