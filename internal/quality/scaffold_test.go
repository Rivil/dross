...
func TestScaffoldRoundTrips(t *testing.T) {
	dir := t.TempDir()
	if err := WriteScaffoldSpec(containedIn(t, dir, "spec.toml"), "07-remediate", "Remediate quality findings", sampleLedger()); err != nil {
		t.Fatal(err)
	}
...
func TestWriteScaffoldSpecWritesIntoRunDirNotCwd(t *testing.T) {
	...
	if err := WriteScaffoldSpec(c, "07-remediate", "Remediate quality findings", sampleLedger()); err != nil {
		t.Fatal(err)
	}
