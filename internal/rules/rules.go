...
var Builtins = []Resolved{
	...
	},
	{
		Scope: "builtin",
		Rule: Rule{
			ID:       "dross-survivor-drain",
			Severity: Hard,
			Text:     "Pre-existing faults are not furniture. When a verify or scan run reports a survivor, finding or FLAG that sits outside the current phase's diff, it must be closed out in the same run — routed to a destination with `dross survivor route --target`, accepted with its reason via `dross survivor accept --reason`, or pulled into a remediation task or scaffolded phase — never re-listed inertly for the next run to inherit. Only an accepted survivor earns silence; routed debt stays visible until it is done. The standing backlog only ever shrinks.",
		},
	},
}
