...
- **No subagent spawns in default mode.** Run inline in main conversation. The user being involved IS the quality gate. Exactly two sanctioned exceptions: `--panel` spawns the three lens planners + one judge (§2P) and nothing else, and §6 hands off to the plan-review flow (which spawns its own cold reviewer). In both, the user steering the result remains the gate.
