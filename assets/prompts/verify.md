...
**A run says which lines it measured, and where it fell back to whole files.** Each leg in tests.json carries `ranges` — the effective, padded line ranges the tool was actually told (`file → [{start, end, pad}]`) — and `whole_file` — every file that leg mutated whole, with its reason (`adapter-lacks-range-runner`, `scope-has-no-hunks`, `file-absent-from-hunks`, `malformed-range`). verify.toml's `[[summary.leg]]` restates them as `pad`, `ranges` and `whole_file`. Run `dross verify scope <phase>` to read the last run's provenance in one place (raw hunks beside effective ranges, per leg), or `--json` for the record itself. A leg with no `ranges` is a whole-file leg and is never called ranged: gremlins always is, and a stryker leg that fell back says why. Judge the score against what was actually instrumented, not against the phase's diff.
