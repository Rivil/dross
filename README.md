...
| `dross milestone {create,list,show,get,set,add,complete}` | Milestones with dotted-path edits (set scalars, add to list fields); `complete` opens the milestone→main PR and (`--finalize`) fast-forwards main + deletes the milestone branch; `show` also accepts `--json` (bare document, no `#` header) | ✅ |
