...
| `dross verify scope <phase>` | Print the last run's range provenance from tests.json: the in-scope files with their raw hunks, then per leg the effective ranges the tool was told (`start-end (pad N)`) and every whole-file fallback with its named reason (`adapter-lacks-range-runner`, `scope-has-no-hunks`, `file-absent-from-hunks`, `malformed-range`). A leg that ranged nothing never prints as ranged, and a record written before provenance existed says so. `--json` emits the same record verbatim; exits non-zero naming `dross verify <phase>` when no run exists | ✅ |
