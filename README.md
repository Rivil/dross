...
| `dross milestone {create,list,show,get,set,add,complete}` | Milestones with dotted-path edits (set scalars, add to list fields); `complete` opens the milestone→main PR and (`--finalize`) fast-forwards main + deletes the milestone branch | ✅ |
| `dross task {next,show,status,add,remove,edit,move}` | Inspect and edit tasks within a plan. `add` appends or inserts (`--after`/`--before`) a task with a fresh high-water id; `remove` is dependency-safe (`--force` strips dependents); `edit` is a partial field update; `move` re-orders a task relative to another. `plan.toml` is mutated only through these verbs, never hand-edited | ✅ |
