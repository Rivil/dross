...
| `dross local {get,set}` | Read/write `.dross/local.toml` — gitignored, machine-local values that must not ride cumulative history. First key: `quick_base`, the branch a standalone `/dross-quick` committed to, which `ship` and `phase complete` reconcile alongside the phase's own base | ✅ |
