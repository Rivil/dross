...
| `dross phase {create,list,show,complete,insert,move,rename,number,migrate}` | Phase directories + slug-based lifecycle. `create` auto-checks out a `phase/<id>` branch off the base so phase work never lands on main; `complete` finalizes after squash-merge (ff base, delete local `phase/<id>`); `insert`/`move`/`rename` edit a milestone's phase array and on-disk identity; `number` prints a phase's ordinal; `migrate` converts legacy NN-slug phases to bare slugs; `show` also accepts `--json` (bare document, no `#` header) | ✅ |
